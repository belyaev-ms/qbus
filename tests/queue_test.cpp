...
BOOST_AUTO_TEST_CASE(one_small_message)
{
    ...
    queue::queue queue(slice_size, 32);
...
BOOST_AUTO_TEST_CASE(remove_message_from_middle_queue)
{
    pmessage_type pmessage;
    queue::queue queue(slice_size, 32);

    buffer_t buffer = make_buffer(slice_size);
    BOOST_REQUIRE_EQUAL(queue.count(), 0);
    do
    {
        pmessage = queue.add(&buffer[0], buffer.size());
    } while (pmessage);
    BOOST_REQUIRE_EQUAL(queue.count(), 32);
    queue.pop(2);
    BOOST_REQUIRE_EQUAL(queue.count(), 30);
    queue.pop();
    BOOST_REQUIRE_EQUAL(queue.count(), 28);
    queue.pop(4);
    BOOST_REQUIRE_EQUAL(queue.count(), 26);
    queue.pop(8);
    BOOST_REQUIRE_EQUAL(queue.count(), 24);
    queue.pop(6);
    BOOST_REQUIRE_EQUAL(queue.count(), 22);
    size_t count = queue.count();
    for (size_t i = 10; i < 32; i+=2)
    {
        queue.pop(i);
        count-=2;
...
BOOST_AUTO_TEST_CASE(client_and_server)
{
    ...
    queue::queue server_queue(&memory[0], slice_size, 32);
    queue::queue client_queue(&memory[0]);
